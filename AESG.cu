...
__device__ const byte CUDA_SBOX[256] = {
    0x63, 0x7C, 0x77, 0x7B, 0xF2, 0x6B, 0x6F, 0xC5, 0x30, 0x01, 0x67, 0x2B, 0xFE, 0xD7, 0xAB, 0x76,
    0xCA, 0x82, 0xC9, 0x7D, 0xFA, 0x59, 0x47, 0xF0, 0xAD, 0xD4, 0xA2, 0xAF, 0x9C, 0xA4, 0x72, 0xC0,
    0xB7, 0xFD, 0x93, 0x26, 0x36, 0x3F, 0xF7, 0xCC, 0x34, 0xA5, 0xE5, 0xF1, 0x71, 0xD8, 0x31, 0x15,
    0x04, 0xC7, 0x23, 0xC3, 0x18, 0x96, 0x05, 0x9A, 0x07, 0x12, 0x80, 0xE2, 0xEB, 0x27, 0xB2, 0x75,
...
__device__ const byte GM11[256] = {
...
__device__ const byte GM13[256] = {
...
__device__ const byte GM14[256] = {
 ...
 */
void RotateWord(byte* w, byte n, byte inv) {
    /*Se hace copia del array*/
    byte tmp[4] = {w[0], w[1], w[2], w[3]};
    int w_it, tmp_it;

    /*Rotamos la palabra segun*/
...
void KeyExpansion(byte * Key) {
    ...
    for (int Round_it = KEY_EXP; Round_it < (ROUNDS + 1)*4; Round_it++) {
        ...
        } else if (KEY_EXP == 8 && Round_it % KEY_EXP == 4) {
...
void initAES(char* op, char* KeyLong, FILE** inputFile,
     ...
     * the expanded key length and the necessary AES rounds. */
    if (*KeyLong == '1') {
        KEY_SIZE = 128;
        ROUNDS = 10;
        KEY_EXP = 4;
 ...
 * @param gpuExpandedKey The array storing the expanded key.
 */
__device__ void AddRoundKey(byte State[4][4], byte round, byte* gpuExpandedKey) {
    int st_f, st_c;

    for (st_f = 0; st_f < 4; st_f++) {
        for (st_c = 0; st_c < 4; st_c++) {
            State[st_f][st_c] ^= gpuExpandedKey[(round * 16) + (st_f * 4) + st_c];
        }
    }
}
...
__device__ void SubBytes(byte State[4][4], byte operation) {
    ...
    for (state_f = 0; state_f < 4; state_f++) {
        for (state_c = 0; state_c < 4; state_c++) {
            ...
            switch (operation) {
                case ENCRYPT:
                    State[state_f][state_c] = CUDA_SBOX[sbox_f * 16 + sbox_c];
                    break;
                case DECRYPT:
 ...
 * @param operation The operation to process (ENCRYPT or DECRYPT).
 */
__global__ void ExecuteAES(byte operation, byte* gpuBuffer, byte* ExpandKey_gpu, int nStates, int rounds) {
    byte State[4][4];

    int round_it, dump_it;
    int thread_idx = blockIdx.x * blockDim.x + threadIdx.x;

    if (thread_idx < nStates) {
        /* In the CUDA version, the State matrix is kernel local, so copy the data form the device
         shared memory into the local matrix*/
        for (dump_it = 0; dump_it < 16; dump_it++) {
            State[dump_it / 4][dump_it % 4] = gpuBuffer[threadIdx.x * 16 + dump_it];
        }
        ...
        for (dump_it = 0; dump_it < 16; dump_it++) {
            gpuBuffer[thread_idx * 16 + dump_it] = State[dump_it / 4][dump_it % 4];
        }
...
int main(int argc, char** argv) {
    ...
    if (argc == 6) {
        ...
        while (bytesRead > 0) {
            ...
            cudaMemcpy(gpuDataBuffer, Buffer, sizeof (byte) * MAX_BUFFER_LENGTH, cudaMemcpyHostToDevice);
            if (cuda_error != cudaSuccess) {
            ...
            ExecuteAES << <65535, 512 >> >(CYPHER_OP, gpuDataBuffer, gpuExpKeyBuffer, nStatesInBuffer, ROUNDS);

            /* Copy the processed data back from the CUDA device's memory to RAM */
            cudaMemcpy(Buffer, gpuDataBuffer, sizeof (byte) * MAX_BUFFER_LENGTH, cudaMemcpyDeviceToHost);
            if (cuda_error != cudaSuccess) {
		...
		printf("AES time elapsed for %s: %.2f ms\n", *argv[1] == 'd' ? "decryption" : "encryption", ms);
